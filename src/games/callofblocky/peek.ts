...
/**
 * Peeking round corners: hold Q to lean out to the left, E to the right (both: upright). The head
 * and shoulders go out sideways (`body.lean`), so the eyes, where shots start, the head's hitbox
 * and the figure everyone sees all go with them, and the view tilts the way it leans. It eases
 * out and back, stops short of a wall on that side, and can't be done sprinting or sliding.
 * A pure step: it runs on the host and, ahead of it, on the player's own screen, so the lean
 * answers the moment the key goes down, online too.
 */
...
export const peek: MovementAbility<PeekState> = {
  ...
  step(s, c, body, dt, world) {
    const side = (c.isDown(PEEK.right) ? 1 : 0) - (c.isDown(PEEK.left) ? 1 : 0);
    const want = body.sprinting || body.sliding || body.flying ? 0 : side * PEEK.reach;
